...
/**
 * 使用LiveData发送消息,与EventBus不同,监听了组件的生命周期,不会发送粘性事件,由于监听了生命周期因此也不支持跨进程
 */
public class DataBus {
    private Map<String,PeekLiveData<?>> liveDataMap;
    public static synchronized DataBus getDefault(){
    ...
    public <T> MutableLiveData<T> with(String key) {
        if (!liveDataMap.containsKey(key)) {
            liveDataMap.put(key, new PeekLiveData<T>());
        }
    ...
    private static class PeekLiveData<T> extends MutableLiveData<T> {
